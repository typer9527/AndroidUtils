...
//        /*如果无法直接打开，就跳出软件列表给用户选择 */
    ...
    public static boolean isOverriedMethod(Object currentObject, String methodName, String method, Class...
            clazzs) {
        LogUtils.i("Info", "currentObject:" + currentObject + "  methodName:" + methodName + "   method:" +
                method);
        boolean tag = false;
    ...
    public static String[] uriToPath(Activity activity, Uri[] uris) {
        ...
        int i = 0;
        for (Uri mUri : uris) {
            paths[i++] = Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN_MR2 ? getFileAbsolutePath
                    (activity, mUri) : getRealPathBelowVersion(activity, mUri);
//            Log.i("Info", "path:" + paths[i-1] + "  uri:" + mUri);
    ...
    static class EncodeFileRunnable implements Runnable {
        ...
        public EncodeFileRunnable(String filePath, Queue<FileParcel> queue, CountDownLatch countDownLatch,
                                  int id) {
        ...
        public void run() {
            ...
            try {
                ...
                if (mFile.exists()) {
                    ...
                    mQueue.offer(new FileParcel(id, mFile.getAbsolutePath(), Base64.encodeToString(os
    ...
    public static Intent getIntentCompat(Context context, File file) {
        ...
        if (context.getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.N) {
            ...
            mIntent.setDataAndType(FileProvider.getUriForFile(context, context.getPackageName() + "" +
                    ".AgentWebFileProvider", file), "application/vnd.android.package-archive");
