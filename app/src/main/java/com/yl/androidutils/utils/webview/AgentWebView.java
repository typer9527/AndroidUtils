...
public class AgentWebView extends WebView implements ChromeClientCallbackManager.AgentWebCompatInterface,
        WebViewClientCallbackManager.PageLifeCycleCallback {
     ...
     * 1、在第一次loadUrl之前注入JS（在addJavascriptInterface里面注入即可，setWebViewClient和setWebChromeClient
     * 要在addJavascriptInterface之前执行）；
     * 2、在webViewClient.onPageStarted中都注入JS；
     * 3、在webChromeClient.onProgressChanged中都注入JS，并且不能通过自检查（onJsPrompt里面判断）JS是否注入成功来减少注入JS的次数，因为网页中的JS
     * 可以同时打开多个url导致无法控制检查的准确性；
     *
     * @deprecated Android4.2.2及以上版本的addJavascriptInterface方法已经解决了安全问题，如果不使用“网页能将JS函数传到Java
     * 层”功能，不建议使用该类，毕竟系统的JS注入效率才是最高的；
     */
    @Override
    @Deprecated
...
//
    ...
    public static Pair<Boolean, String> isWebViewPackageException(Throwable e) {
        ...
        if (trace.contains("android.content.pm.PackageManager$NameNotFoundException")
                ...
                || trace.contains("android.webkit.WebViewFactory$MissingWebViewPackageException: Failed to " +
    ...
    public void onProgressChanged(WebView view, int newProgress) {
        if (mJsCallJavas != null) {
            injectJavaScript();
            if (LogUtils.isDebug()) {
                Log.d(TAG, "injectJavaScript, onProgressChanged.newProgress = " + newProgress + ", url = "
                        + view.getUrl());
            }
        }
        if (mInjectJavaScripts != null) {
    ...
    private void fixedStillAttached() {
        ...
        if (parent instanceof ViewGroup) { //
            // 由于自定义webView构建时传入了该Activity的context对象，因此需要先从父容器中移除webView，然后再销毁webView；
    ...
    private void releaseConfigCallback() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) { // JELLY_BEAN
            ...
            try {
                Field sConfigCallback = Class.forName("android.webkit.BrowserFrame").getDeclaredField
                        ("sConfigCallback");
...
//                    URLEncodedUtils.parse(new URI(url), null); // AccessibilityInjector
// .getAxsUrlParameterValue
                } catch (IllegalArgumentException e) {
                    if ("bad parameter".equals(e.getMessage())) {
                        mIsAccessibilityEnabledOriginal = true;
                        setAccessibilityEnabled(false);
                        LogUtils.i(TAG, "fixedAccessibilityInjectorExceptionForOnPageFinished" +
                                ".url = " + url + "eexception" + e);
                    }
                }
            } catch (Throwable e) {
                if (LogUtils.isDebug()) {
    ...
    private boolean isAccessibilityEnabled() {
        AccessibilityManager am = (AccessibilityManager) getContext().getSystemService(Context
    ...
    private void setAccessibilityEnabled(boolean enabled) {
        AccessibilityManager am = (AccessibilityManager) getContext().getSystemService(Context
                .ACCESSIBILITY_SERVICE);
        try {
            Method setAccessibilityState = am.getClass().getDeclaredMethod("setAccessibilityState", boolean
                    .class);
