...
public class FragmentUtils {
    ...
    public static Fragment hideAddFragment(@NonNull final FragmentManager fragmentManager,
        ...
        return operateFragment(fragmentManager, hideFragment, addFragment, TYPE_HIDE_ADD_FRAGMENT,
                sharedElement);
    }
        ...
        for (int i = 0, size = fragments.size(); i < size; ++i) {
            ...
            if (fragment != null) {
                if (list != null) {
                    ...
                    return operateFragment(fragmentManager, null, fragment, TYPE_ADD_FRAGMENT, list.toArray
                            (new SharedElement[0]));
                }
            }
        }
    ...
    public static boolean popToFragment(@NonNull final FragmentManager fragmentManager,
        ...
        return fragmentManager.popBackStackImmediate(fragmentClass.getName(), isIncludeSelf ?
    ...
    private static Args getArgs(@NonNull final Fragment fragment) {
        ...
        return new Args(bundle.getInt(ARGS_ID), bundle.getBoolean(ARGS_IS_HIDE), bundle.getBoolean
                (ARGS_IS_ADD_STACK));
    }
...
//            LogUtils.e(srcFragment.getClass().getName() + " is isRemoving");
    ...
    private static Fragment getTopShowFragmentIsInStack(@NonNull final FragmentManager fragmentManager,
        ...
        for (int i = fragments.size() - 1; i >= 0; --i) {
            Fragment fragment = fragments.get(i);
            if (fragment != null && fragment.isResumed() && fragment.isVisible() && fragment
                    .getUserVisibleHint()) {
                if (isInStack) {
                    if (fragment.getArguments().getBoolean(ARGS_IS_ADD_STACK)) {
     ...
     * @return 栈中同级别fragment
     */
    private static List<Fragment> getFragmentsIsInStack(@NonNull final FragmentManager fragmentManager,
                                                        final boolean isInStack) {
    ...
    private static List<FragmentNode> getAllFragmentsIsInStack(@NonNull final FragmentManager fragmentManager,
        ...
        for (int i = fragments.size() - 1; i >= 0; --i) {
            ...
            if (fragment != null) {
                if (isInStack) {
                    if (fragment.getArguments().getBoolean(ARGS_IS_ADD_STACK)) {
                        result.add(new FragmentNode(fragment, getAllFragmentsIsInStack(fragment
                                .getChildFragmentManager(), new ArrayList<FragmentNode>(), true)));
                    }
                } else {
                    result.add(new FragmentNode(fragment, getAllFragmentsIsInStack(fragment
                            .getChildFragmentManager(), new ArrayList<FragmentNode>(), false)));
                }
            }
        }
        return result;
     ...
     * @return 查找到的fragment
     */
    public static Fragment findFragment(@NonNull final FragmentManager fragmentManager, final Class<?
            extends Fragment> fragmentClass) {
        List<Fragment> fragments = getFragments(fragmentManager);
        if (fragments.isEmpty()) return null;
    ...
    static class FragmentNode {
        ...
        public String toString() {
            return fragment.getClass().getSimpleName() + "->" + ((next == null || next.isEmpty()) ? "no " +
                    "child" : next.toString());
        }
    }
