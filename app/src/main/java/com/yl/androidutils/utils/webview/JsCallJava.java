...
public class JsCallJava {
    ...
    private static final String[] IGNORE_UNSAFE_METHODS = {"getClass", "hashCode", "notify", "notifyAll",
            "equals", "toString", "wait"};
    ...
    public JsCallJava(Object interfaceObj, String interfaceName) {
        try {
            ...
            sb.append(" init begin\");var a={queue:[],callback:function(){var d=Array.prototype.slice.call" +
                    "(arguments,0);var c=d.shift();var e=d.shift();this.queue[c].apply(this,d);if(!e)" +
                    "{delete this.queue[c]}}};");
            ...
            sb.append(mInterfacedName);
            sb.append(" call error, message:miss method name\"}var e=[];for(var h=1;h<f.length;h++){var " +
                    "c=f[h];var j=typeof c;e[e.length]=j;if(j==\"function\"){var d=a.queue.length;a" +
                    ".queue[d]=c;f[h]=d}}var k = new Date().getTime();var l = f.shift();var m=prompt('");
            sb.append(MSG_PROMPT_HEADER);
            sb.append("'+JSON.stringify(");
            sb.append(promptMsgFormat("'" + mInterfacedName + "'", "l", "e", "f"));
            sb.append("));console.log(\"invoke \"+l+\", time: \"+(new Date().getTime()-k));var g=JSON.parse" +
                    "(m);if(g.code!=200){throw\"");
            sb.append(mInterfacedName);
            sb.append(" call error, code:\"+g.code+\", message:\"+g.result}return g.result};Object" +
                    ".getOwnPropertyNames(a).forEach(function(d){var c=a[d];if(typeof " +
                    "c===\"function\"&&d!==\"callback\"){a[d]=function(){return c.apply(a,[d].concat(Array" +
                    ".prototype.slice.call(arguments,0)))}}});b.");
            sb.append(mInterfacedName);
    ...
    public String call(WebView webView, JSONObject jsonObject) {
        ...
        if (jsonObject != null) {
            try {
                ...
                // 方法匹配失败
                if (currMethod == null) {
                    return getReturn(jsonObject, 500, "not found method(" + sign + ") with valid " +
                            "parameters", time);
                }
                // 数字类型细分匹配
                if (numIndex > 0) {
    ...
    private String getReturn(JSONObject reqJson, int stateCode, Object result, long time) {
        ...
        if (result == null) {
            ...
            // 兼容：如果在解决WebView注入安全漏洞时，js注入采用的是XXX:function(){return prompt(...)
            // }的形式，函数返回类型包括：void、int、boolean、String；
        ...
        if (LogUtils.isDebug()) {
            Log.d(TAG, "call time: " + (android.os.SystemClock.uptimeMillis() - time) + ", request: " +
                    reqJson + ", result:" + resStr);
        }
        return resStr;
    }
